...
string CPU::getisHLTInstruction() const {
    return isHLTInstruction;
}
...
void CPU::setisHLTInstruction(string _instruction) {
    isHLTInstruction = _instruction;
}
void CPU::setOption(int theOption){
    if(theOption>2)
...
void CPU::print() const {
    ...
    cout << endl << "CPU Register Values:" << endl;
    for (int index = 0; index < REGISTER_SIZE; ++index)
        cout << "R" << index + 1 << ":" << registers[index] << endl;
}

void CPU::print(int ignore) const {
    for (int index = 0; index < REGISTER_SIZE; ++index)
...
bool CPU::execute(string theInstruction, Memory& memoryObj) {
    ...
    if (opcode == "HLT") {
        ...
        if (found != string::npos)
            JumpInstructionWithTwoOperand(theInstruction, memoryObj);

        else //if instruction like : JMP lineAdress
            JumpInstructionWithOneOperand(theInstruction, memoryObj);
        if (PC > getHLTIndex() || PC < ZERO) {
            ExitProgram("JMP ERROR: Out of the range!");
        }
    } else if (opcode == "JPN") {
        JPNInstruction(theInstruction, memoryObj);
    } else if (opcode == "ADD") {
...
void CPU::JPNInstruction(string instruction, Memory& memoryObj) {
    ...
    if (!(leftOperand.size() > 0))
        ExitProgram("JPN Instruction: There is no right operand!");
...
void CPU::JumpInstructionWithTwoOperand(string instruction, Memory& memoryObj) {
    size_t indexOfComma = instruction.find_first_of(",");
...
void CPU::JumpInstructionWithOneOperand(string instruction, Memory& memoryObj) {
    string operand;
    string unnecessary;
...
void CPU::AddInstruction(string instruction, Memory& memoryObj) {
    ...
    //if option is 1 prints registers value and PC to screen
    if (option) {
